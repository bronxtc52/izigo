...
6. **Нет заявок на вывод в processing** (held в работе): дождаться завершения/паузы, иначе
   ...
   **⛔ Операционный предохранитель (поверх TOCTOU-фикса): на окно cutover ЗАМОРОЗИТЬ одобрение
   и создание выводов (withdrawals).** Код закрывает гонку атомарно (held-предохранитель
   повторяется ВНУТРИ транзакции миграции под ACTIVATION_LOCK + `lockForUpdate` на кошельке:
   вывод, созданный между пре-чеком и стартом транзакции, откатывает ВСЮ миграцию и пишет
   `phase=rolled_back` в `v2_cutover_log`). Но авторитетный аборт означает повторный ручной
   прогон — дешевле не создавать выводы в узкое maintenance-окно вообще: предупредить оператора
   поддержки «не одобрять/не проводить выводы, пока не дан сигнал завершения cutover».
...
9. **Bronze 90→100 применён и переживает redeploy** (`ProductSeeder` в `docker/start.sh` НЕ
   откатывает тариф на 90 при следующем деплое/рестарте; флаг движка `mh_plan_v2_engine` тоже
   переживает рестарт — ни один сидер его не трогает). Инвариант зафиксирован тестами
   (`CutoverMigrateTest::test_product_seeder_does_not_revert_bronze_after_cutover`,
   `SeederEngineFlagGuardTest`).
