...
- [ ] **F4 — Autoship.** `GET/POST /cabinet/autoship`, `PATCH /cabinet/autoship/{id}`
  (pause/resume/cancel): список подписок (next_charge_at/interval/status), создать, управлять.
  Сценарий: автоповтор покупки для поддержания активности.
- [ ] **F5 — Пополнение баланса.** `POST /cabinet/wallet/topup` → тот же checkout-флоу TON Pay; размещаем
  рядом с кошельком во вкладке «Доход» (`MiniAppShell.js`). Сценарий: пополнить внутренний USDT-баланс.
- [ ] **F6 — KYC + TON-адрес вывода.** `GET /cabinet/kyc`, `POST /cabinet/kyc/passport` (Telegram
  Passport кнопка + статус none/pending/approved/rejected+reason). Проверить, принимает ли
  `POST /cabinet/withdrawals` поле `ton_address`; если нет — это backend-гап (вернуться к Гейту 2/3 по
  бэку), иначе добавить поле TON-адреса в модалку вывода. Сценарий: пройти верификацию и указать адрес
  для выплат.
- [ ] **F7 — Админ-экраны.** Расширить `MiniAppAdmin`/`src/views/admin/*`: товары CRUD
  (`admin/products`), заказы+смена статуса/трекинг (`admin/orders`, `PATCH .../status`), KYC-очередь
  (`admin/kyc`, `PATCH .../{id}` approve/reject), on-chain выплата (`AdminWithdrawals` + кнопка «Отправить»
  `admin/withdrawals/{id}/send` + показ `tx_hash`). Сценарий: оператор ведёт каталог, исполнение, KYC,
  выплаты.
- [~] **F8 — Гейт 4.** Для куска **F0–F3** пройдено: `reviewer` (независимый) → 0×P0, 4×P1, P2; правки
  применены (таймаут поллинга, обработка `{error}`, guard на смену инвойса, кнопка «Оплатить» для
  висящих заказов, единый fallback манифеста, вынос `usd`→`format.js`) → `npm run build` зелёный.
  ⏳ Осталось: ручной клик-тест в Telegram (за пользователем, против `ton_pay_fake`) + повтор Гейта 4 для
  F4–F7. Деплой — НЕ в этой сессии (нужны боевые TON-драйверы + секреты, см. `izigo-phase4-progress`).
...
4. [~] `admin_audit_log` таблица+модель+`AuditLogService`+`GET /admin/audit-log`; запись в
       мутациях: **план + роли (assign/revoke) — готово**; выплаты/продукты/KYC — хвост блока B.
