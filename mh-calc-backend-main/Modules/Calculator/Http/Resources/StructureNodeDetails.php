 ...
 * @mixin Node
 */
class StructureNodeDetails extends JsonResource
{
    private Structure $structure;

    public function __construct($resource, Structure $structure)
    {
    ...
    public function toArray(Request $request)
    {
        ...
        return [
            ...
            'details' => $this->getDetails($currency),
            'log' => $this->getLog()
        ];
    }

    private function getDetails(LocaleEnum $currency): array
    {
        $result = [
            'rank_list' => $this->rank_list,
            'bonus_list' => [
                'referral' => [
                    'title' => __("calculator::marketing.title_bonus_referral"),
                    'total' => CurrencyFormatter::fiat($this->bonus_referral_sum, $currency->currency),
                    'total_by_level' => (object)$this->getReferralBonusByLevels($currency->currency),
                    'list' => [],
                ],
                'binary' => [
                    'title' => __("calculator::marketing.title_bonus_binary"),
                    'total' => CurrencyFormatter::fiat($this->bonus_binary_sum, $currency->currency),
                    'list' => [],
                ],
                'leader' => [
                    'title' => __("calculator::marketing.title_bonus_leader"),
                    'total' => CurrencyFormatter::fiat($this->bonus_leader_sum, $currency->currency),
                    'list' => [],
                ],
                'rank' => [
                    'title' => __("calculator::marketing.title_bonus_rank"),
                    'total' => CurrencyFormatter::fiat($this->bonus_rank_sum, $currency->currency),
                    'list' => [],
                ],
            ]];

        /** @var BonusDataReferral $bonus */
        foreach ($this->bonus_referral_list as $bonus) {
            $result['bonus_list']['referral']['list'][] = $bonus->getDetail();
        }

        /** @var BonusDataBinary $bonus */
        foreach ($this->bonus_binary_list as $bonus) {
            $result['bonus_list']['binary']['list'][] = $bonus->getDetail();
        }

        /** @var BonusDataLeader $bonus */
        foreach ($this->bonus_leader_list as $bonus) {
            $result['bonus_list']['leader']['list'][] = $bonus->getDetail();
        }

        /** @var BonusDataRank $bonus */
        foreach ($this->bonus_rank_list as $bonus) {
            $result['bonus_list']['rank']['list'][] = $bonus->getDetail();
        }
    ...
    private function getLog(): array
    {
        ...
        {
            /** @var EventData $bonus */
            foreach ($this->logEvents->eventList as $index => $eventsGroup) {
                $result[$index] = [];
                foreach ($eventsGroup as $event) {
                    $result[$index][] = $event->getDetail();
                }
            }
        }
